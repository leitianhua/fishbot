...
(function() {
	...
	// === 新增：覆盖 fetch 方法以监听包含 itemId 和 sessionId 的请求 ===
	// 保存原始的open和send方法
    ...
    XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
        // 检查URL是否包含目标API
        this.isTargetRequest = url.includes('mtop.idle.trade.pc.message.headinfo');
    ...
    XMLHttpRequest.prototype.send = function(data) {
        ...
        if (this.isTargetRequest) {
            ...
            try {
                ...
                // 提取 JSON 字符串
                const jsonDataMatch = decodedData.match(/data=({.*})/);
                if (jsonDataMatch && jsonDataMatch[1]) {
                    const jsonData = jsonDataMatch[1]; // 提取 JSON 字符串
                    const dataObj = JSON.parse(jsonData); // 解析为 JSON 对象

                    const itemId = dataObj.itemId;
                    const sessionId = dataObj.sessionId;

                    if (itemId) {
                        console.log('当前itemId:', itemId);
                    }
                    //if (sessionId) {
                    //    console.log('sessionId:', sessionId);
                    //}
                } else {
                    console.warn('未找到有效的 JSON 数据:', decodedData);
                }
            } catch (error) {
    ...
    async function waitForMessages() {
        log('等待消息加载...');
        for (let i = 0; i < 10; i++) {
            const messageRows = document.querySelectorAll('[class^="message-row"]');
    ...
    async function checkNewMessageBadge() {
        ...
        if (unreadBadge) {
            log(`找到消息角标，待回复消息数量: ${unreadBadge.textContent}`);
    ...
    function extractMessages() {
        ...
        messageRows.forEach((row, index) => {
            const messageContent = row.querySelector('[class*="message-text"]');
            if (messageContent) {
                //const isOwnMessage = messageContent.classList.contains('[class*="message-text-right"]');
				const isOwnMessage = messageContent.className.includes('message-text-right')
                const content = messageContent.textContent.trim();
                const sender = isOwnMessage ? '卖家:' : '客户:';
                messages.push({
